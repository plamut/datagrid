...
(\TODO{ref.}) psevdokode s Slike~\ref{alg:MFS}:
\begin{equation}
...
\begin{samepage}
Formulo za izračun vrednosti skupine replik najdemo na levi strani
neenakosti v vrstici 18 (\TODO{ref.}):
\begin{equation}
	\sum_{y=1}^{x-1} \mathit{NORList(y)}
  \label{eq:MFS_GV}
\end{equation}
pri čemer je pomen posameznih spremenljivk pojasnjen v
Tabeli~\ref{tbl:MFS_vars}.
...
\TODO{komentar - pač seštevek NOR in to je to}



\TODO{kot opombo pa omeni urejenost replik v ReplicaList, da niso urejene
po RV, ampak
malenkost drugače (referenca na članek)}
