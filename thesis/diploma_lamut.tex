...
\begin{algorithm}[H]
  \label{alg:EFS}
  \caption{Algoritem EFS (\TODO{preimenuj bold oznako})}
...
\blankln{}
                      ...
                      $GV = \frac{\sum\limits_{y=1}^{x-1} NORList(y)}{
                                  \sum\limits_{y=1}^{x-1} SizeList(y)} +
                            \frac{\sum\limits_{y=1}^{x-1}
                              \mathit{NORFSTIList(y)}}{\mathit{FSTI}} +
                            \frac{1}{CT - \frac{\sum\limits_{y=1}^{x-1}
                              LRTList(y)}{x-1}}$

\blankln{}

                      $RV_{RR} = \frac{\mathit{NORRR}}{\mathit{SRR}} +
...
\blankln{}
                      ...
                      \If{$\mathit{GV} < \mathit{RV_{RR}}$} {
                          \For{$y = 1$ \KwTo $x-1$}{
                            Delete ReplicaList(y), NORList(y), SizeList(y),
                            NORFSTIList(y), LRTList(y)\;
                          }
                          Copy RR;
                      }
                  }
...
%\TODO{vse variable italic, kjer niso! Npr. ReplicaList - define \Var{name}}

\TODO{tabela kratic iz psevdokode, kaj kaj pomeni}

tukaj opiši, kako izračunamo group value in kako replica value
(iz članka 2011) GV, RV, pomen, interpretacija, tabela pojmov (kratic) v
formuli itd.
